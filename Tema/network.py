...
def regenerate_code(data):
    ...
    db.session.commit()
    emit('code_regeneration', {'classroom_id': data['classroom_id'],
                               'code': Classroom.query.filter(Classroom.id == data['classroom_id']).first().code})
...
def handle_conversations(data):
    if current_user.is_authenticated:
        if len(data['message']):
            ...
            if Membership.query.filter(Membership.user_id == sender_id,
                                       Membership.classroom_id == room.classroom_id).first():
                new_message = Message(author_id=sender_id, channel_id=data['channel_id'], contents=data['message'])
                emit('channel_conversation', {'content': data['message'], 'date': 'just now',
                                              'author': {'id': sender_id,
                                                         'name': sender.first_name + ' ' + sender.last_name},
                                              'channel_id': data['channel_id']}, room=str(room.classroom_id))
                # Saving the message in the db
                db.session.add(new_message)  # adding message
                db.session.commit()
        else:
            emit('error', 'message must have more than one character.')
...
def channel_action(data):
    membership = Membership.query.filter(Membership.user_id == current_user.get_id(),
                                         Membership.classroom_id == data['classroom_id']).first()
    if membership and membership.role == 'super':
        if data['action'] == 'add':
            new_channel = Channel(classroom_id=int(data['classroom_id']), name=data['name_input'])
            db.session.add(new_channel)
            db.session.flush()
            db.session.commit()
            emit('new_channel', {'classroom_id': data['classroom_id'], 'name': new_channel.name, 'id': new_channel.id},
                 room=str(data['classroom_id']))
            ...
            db.session.commit()
            emit('channel_rename',
                 {'classroom_id': data['classroom_id'], 'new_name': data['name_input'], 'id': data['channel_id']},
                 room=str(data['classroom_id']))
            ...
            db.session.commit()
            emit('channel_delete', {'classroom_id': data['classroom_id'], 'id': data['channel_id']},
                 room=str(data['classroom_id']))
    else:
        pass
...
def user_action(data):
    ...
    if membership.role == 'super':
        if data['action'] == 'kick':
            Membership.query.filter(Membership.classroom_id == int(data['classroom_id']),
                                    Membership.user_id == int(data['user_id'])).delete()
            db.session.commit()
            emit('user_kick', {'user_id': data['user_id'], 'classroom_id': data['classroom_id']},
                 room=str(data['classroom_id']))
...
def user_leave(data):
    if current_user.is_authenticated:
        Membership.query.filter(Membership.user_id == current_user.get_id(),
                                Membership.classroom_id == data['classroom_id']).delete()
        db.session.commit()
...
def direct_message(data):
    ...
    if recipient_username in users.keys():
        emit('direct_message', {'content': data['message'],
                                'author': User.query.filter(User.id == current_user.get_id()).first().username,
                                'date': 'Just now'}, room=users[recipient_username])
    emit('direct_message',
         {'content': data['message'], 'author': User.query.filter(User.id == current_user.get_id()).first().username,
          'date': 'Just now'}, room=users[User.query.filter(User.id == current_user.get_id()).first().username])
